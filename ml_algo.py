...
X = dataset.iloc[:, [1,3,4,20,21,22,23,25,26,27,28,29]].values   #Sound,temp,humidity as i/p
#X = dataset.iloc[:, [1,3,4,9,20,21,22,23,25,26,27,28,29]].values #Sound,temp,humidity,motion as i/p
#X = dataset.iloc[:, [1,3,4,6,7,8,20,21,22,23,25,26,27,28,29]].values #Sound,temp,humidity,accelorometer as i/p
...
classifier = RandomForestClassifier(n_estimators = 100, criterion = 'entropy',max_depth=10,min_samples_leaf=10, random_state = 0)
...
classifier.add(Dense(output_dim = 8, init = 'uniform', activation='relu', input_dim = 12)) 
#adding dropouts
...
#Compiling the ANN
#opt = SGD(lr=0.01, momentum=0.9,decay=0.0001)                             #Gradient Descent with moemntum optimization
#opt = keras.optimizers.RMSprop(learning_rate=0.001)                       #RMS optimation
classifier.compile(optimizer='adam',loss = 'sparse_categorical_crossentropy', metrics=['accuracy']) #Adam optimization
...
#history=classifier.fit(X_train, y_train, validation_data=(X_test, y_test), epochs =1000, callbacks=[es,mc]) #with early stopping
